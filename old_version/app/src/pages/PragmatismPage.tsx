...
const pragmatismPosts = getArticlesByCategory('实用主义研究')
          ...
          {pragmatismPosts.map((article, i) => (
            <div key={article.id} ref={(el) => { if (el) itemRefs.current[i] = el }} style={{ borderBottom: '1px solid var(--color-border)', padding: '32px 0' }}>
              ...
              <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(18px, 2vw, 24px)', fontWeight: 400, lineHeight: 1.4, marginBottom: '12px', color: 'var(--color-text)', transition: 'color 0.3s ease' }}>
                <Link to={`/article/${article.id}`} style={{ color: 'inherit', textDecoration: 'none' }} onMouseEnter={(e) => { e.currentTarget.style.color = 'var(--color-accent)' }} onMouseLeave={(e) => { e.currentTarget.style.color = 'inherit' }}>
                  {article.title}
                </Link>
              </h2>
